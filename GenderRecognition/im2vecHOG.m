...
for i = 1:6:55
    for j=1:6:55
        ...
        for a= 1:6
            for b=1:6
                index=floor(degree1(a,b)/3);
                mg(index+1)=mg(index+1)+Gmag1(a,b)-Gmag1(a,b)*mod(degree1(a,b),3)/3;
                mg(index+2)= mg(index+2)+Gmag1(a,b)* mod(degree1(a,b),3)/3;      
...
%                 mg(index+2)= mg(index+2)+Gmag1(a,b)* mod(degree1(a,b),20)/20;                
        ...
        mg(1)=mg(1)+mg(121);
%         for k=1:18
%         veclength = veclength+mg(k)*mg(k);
%         end
%         veclength = sqrt(veclength);
%         mg=mg./veclength;
        cellHis=[cellHis; mg(1:120)];
    end
end
