...
face_folder= 'F:\Face_Detection_code\FACE\';
non_face_folder= 'F:\Face_Detection_code\NONFACE\';
so_face= 700; % so face
so_non_face= 1400; % so non face
...
fprintf('\nTai anh khuon mat');
 ...
 for k=1:so_face
    ...
    IM {1} = im2vec1 (image);    % vecto dac trung cua anh goc
    IM {2} = im2vec1 (fliplr(image));    % vector dac trung cho anh doi xung truc 
   for i=1:2
