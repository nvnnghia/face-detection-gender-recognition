...
RectCoord = [xCoord-30 yCoord-30 60 60];
imcut=(im1(yCoord-29:yCoord+30,xCoord-29:xCoord+30));
a= svmclassify(netHOG1,im2vecHOG(imcut));
        if (a==1)
            FaceCoordMale = [FaceCoordMale; RectCoord];
        else
            FaceCoordFeMale = [FaceCoordFeMale; RectCoord];
%             figure
%             imshow(imcut,[]);
        end
...
RGB = insertObjectAnnotation(im,'rectangle',FaceCoordMale,'NAM');
RGB = insertObjectAnnotation(RGB,'rectangle',FaceCoordFeMale,'NU');
figure
imshow(RGB)
% figure;
% imshow(im);
